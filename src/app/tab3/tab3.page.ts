...
export class Tab3Page {
  ...
  opcionSeleccionada: string = 'page1';
  ...
  mostrarDiv(): void {
    ...
    this.paginaActual = parseInt(this.opcionSeleccionada);
  }
  ...
  async saveDocumentalAnswers() {
    ...
    const data = {
      ...
      rel_type: type,
      id_checklist: this.documental_checklist_id ?? 31,
          ...
          .then((observableResult) => {
            observableResult.subscribe(
              (response: any) => {
                ...
                this.client_name = response.plan_detail[0].client;
          ...
          .then((observableResult) => {
            observableResult.subscribe(
              (response: any) => {
                ...
                this.uvTotalTasks = response.tasks;
              },
              (error: any) => {
                console.error('Error al enviar datos:', error);
              }
              ...
              (response: any) => {
                ...
                for (const sectionId in response.items) {
                  if (response.items.hasOwnProperty(sectionId)) {
                    ...
                    // Verificar si sectionItems es un array antes de asignarlo
  ...
  setAnswerDocumental(idItemChecklist: string, answer: any, description = '') {
    setTimeout(() => {
      this.checklistService.setSelectedItem(
        idItemChecklist,
        answer,
    ...
    // this.documental_checklist_answers.push({
    //   id: idItemChecklist,
    //   answer: 'yes',
    //   description: '',
    // });
  ...
  getSelectedProjectItem(itemId: string): {} | undefined {
  ...
  showTicketsModal(open: boolean) {
    ...
    const data = {
      ...
      user_id: this.userdata.staffid,
    };
    this.httpService
      .post(
        `staffs/${this.userdata.staffid}/get_ticket_data`,
        JSON.stringify(data),
        true
      )
      .then((observableResult) => {
  ...
  async createTicket() {
    ...
    if (validate) {
      ...
      const url = await this.uploadImage(blob, 'ticketEvidence');
      const data = {
  ...
  getSelectedOptionsTechnicians(event: any, index: any) {
    ...
    selectedValues.length < 6
      ? (this.techniciansResponse = 0)
      : (this.techniciansResponse = 1);
    this.selectedTextsTechnicians[index] = 'validado';
  }
  getSelectedOptionsProducts(event: any, index: any) {
    const selectedValues = event.detail.value;
    selectedValues.length < 4
      ? (this.productsResponse = 0)
      : (this.productsResponse = 1);
    this.selectedTexts[index] = 'validado';
  }
  ...
  answerTaskUvChecklist(value: string, question_id: string) {
    this.checklistUvService.setSelectedItem(question_id, value);
  }
