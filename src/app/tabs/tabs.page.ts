...
export class TabsPage {
  selectedOption: string = ''; // Opción por defecto
  ...
  constructor(
    ...
    this.authservice
      ...
      .then((userData: any) => {
        ...
        this.httpService
          .get(
            `staffs/${this.userdata.staffid}/get_subsidiaries_for_staff`,
            true
          )
          .then((observableResult) => {
            observableResult.subscribe(
  ...
  handleSubsidiarySelect(event: any) {
    this.subsidiarySelect = event.detail.value;
    this.ticketType = 'subsidiary';
  ...
  handleControl(event: any) {
    ...
    this.httpService
      .post(
        `staffs/${this.userdata.staffid}/getTasks`,
        JSON.stringify(data),
        true
      )
      .then((observableResult) => {
  ...
  handleCinturon(event: any) {
    ...
    this.httpService
      .post(
        `staffs/${this.userdata.staffid}/get_trampas`,
        JSON.stringify(data),
        true
      )
      .then((observableResult) => {
